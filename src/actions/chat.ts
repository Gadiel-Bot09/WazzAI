...
export async function getConversationsAction(): Promise<ActionResult<{ conversations: any[]; showAssignedAgent: boolean; orgId: string; currentUser: { id: string; departmentId: string | null; role: string; permissions: any; status: string } }>> {
  ...
  // Get user's team member data to know their department and permissions
  const { data: teamData } = await supabase.from('team_members').select('*, roles(name, permissions)').eq('user_id', user.id).eq('org_id', profile.org_id).maybeSingle()
  ...
  return ok({ conversations, showAssignedAgent, orgId: profile.org_id, currentUser: { id: user.id, departmentId: teamMember?.department_id, role: teamMember?.roles?.name || 'agent', permissions: teamMember?.roles?.permissions || {}, status: teamMember?.status || 'offline' } })
}
